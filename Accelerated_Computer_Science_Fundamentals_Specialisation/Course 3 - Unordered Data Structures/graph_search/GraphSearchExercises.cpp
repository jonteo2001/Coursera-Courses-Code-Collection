...
int GridGraph::countEdges() const {
  ...
  // =======================================================================
  // TODO: Your code here!
    //Create the variable edgeSet
    std::unordered_set<IntPairPair> edgeSet;
    for (const auto& kv : adjacencyMap) // Loop over key value pairs
    // Adapted from printDetails in line 176 (GridGraph.cpp)
    {
        //Create a variable for the key
        const auto& p1 = kv.first;
        //Value: Neighbour point SET - it is a set
        const auto& p1_neighbours = kv.second;
        if (p1_neighbours.empty()) continue;
        else
        {
            for (const auto& p2 : p1_neighbours)
            {
                IntPairPair edge;
                if (p1 < p2)
                {
...
std::list<IntPair> graphBFS(const IntPair& start, const IntPair& goal, const GridGraph& graph) {
  ...
  std::unordered_map<IntPair, IntPair> pred;//see dijkstra algo slide 3
  ...
  std::unordered_map<IntPair, int> dist;//see dijkstra algo slide 3
...
std::list<PuzzleState> puzzleBFS(const PuzzleState& start, const PuzzleState& goal) {
  ...
  while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
    ...
    auto neighbors = curState.getAdjacentStates(); //Line 254: PuzzleState.h
      //i need to find an unordered map for puzzlestate.
