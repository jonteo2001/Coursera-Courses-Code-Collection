...
//==========================================================================
// REMEMBER ITS NOT AN ARRAY ITS AN UNORDERED MAP
...
StringIntMap makeWordCounts(const StringVec& words)
{
    ...
    {
        if (wordcount_map.count(words[i])) // Check if word exists in dictionary
        {
            wordcount_map[words[i]]+=1; // if it is add 1 to the word count
        }
        else
        {
            wordcount_map[words[i]] = 1 ; // if not set its value to be 1
        }
    }
    // =================================================
...
//
// People commonly use the indexing [] operator with maps to conveniently do both
...
int lookupWithFallback(const StringIntMap& wordcount_map, const std::string& key, int fallbackVal) {
  ...
  // EXERCISE 2 WORKSPACE: YOUR CODE HERE
    if (wordcount_map.count(key)) //boolean true or false
    {
        StringIntMap::const_iterator it = wordcount_map.find(key);
        return (it->second);
    }
...
int memoizedLongestPalindromeLength(LengthMemo& memo, const std::string& str, int leftLimit, int rightLimit, timeUnit startTime, double maxDuration) {
  ...
  if (memo.count(pairKey)) {
    ...
    // this case, only return what's already stored at this key in the map.
      LengthMemo::const_iterator it = memo.find(pairKey);
    return it->second; // Hint: You need to change this!
    // ====================================================================
  }
