...
void printVector(vector<int> vec, string name = "NULL")
{
    cout << " -----------" << name << "------------" << endl;
    for (auto &v: vec)
    {
...
vector<int> BuildSuffixArray(const string& text) {
  ...
  compute_class(text, order, sort_class);
    printVector(sort_class, "Sort_Class");
  for (int l = 1; l <= text.size(); l *= 2) {
    order = sort_double(text, l, order, sort_class);
      printVector(order, "Order after sort double");
    sort_class = update_class(order, sort_class, l);
      printVector(sort_class, "sort_class after update");
  }
